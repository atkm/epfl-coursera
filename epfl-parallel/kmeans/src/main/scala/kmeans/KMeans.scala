...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val classfied = points groupBy { p => findClosest(p, means) }
    val noMissingMean = means map { p => if (classfied contains p) (p, classfied(p)) else (p, Nil) }
    noMissingMean.toMap
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    oldMeans map { p =>
      val cluster = classified(p)
      findAverage(p, cluster)
    }
  }
